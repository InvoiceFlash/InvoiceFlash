<?php
// Error DIR_INVOICEFLASH
error_reporting(E_ALL);

// HTTP
$protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
define('HTTP_SERVER', $protocol . $_SERVER['HTTP_HOST'] . rtrim(dirname($_SERVER['SCRIPT_NAME']), '/.\\') . '/');
define('HTTP_INVOICEFLASH', $protocol . $_SERVER['HTTP_HOST'] . rtrim(rtrim(dirname($_SERVER['SCRIPT_NAME']), 'install'), '/.\\') . '/');

// DIR
define('DIR_APPLICATION', str_replace('\'', '/', realpath(dirname(__FILE__))) . '/');
define('DIR_SYSTEM', str_replace('\'', '/', realpath(dirname(__FILE__) . '/../')) . '/system/');
define('DIR_INVOICEFLASH', str_replace('\'', '/', realpath(DIR_APPLICATION . '../')) . '/');
...
if (file_exists('../config.php')) {
	if (filesize('../config.php') > 0) {
		...
		$lines = file(DIR_INVOICEFLASH . 'config.php');
