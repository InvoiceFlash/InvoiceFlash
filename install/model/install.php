...
class ModelInstall extends Model {
	public function database($data) {
		...
		if ($lines) {
			...
			foreach($lines as $line) {
				...
				if ($line && (substr($line, 0, 2) != '--') && (substr($line, 0, 1) != '#')) {
					...
					if (preg_match('/;\s*$/', $line)) {
						$sql = str_replace("DROP TABLE IF EXISTS `if_", "DROP TABLE IF EXISTS `" . $data['db_prefix'], $sql);
						$sql = str_replace("CREATE TABLE `if_", "CREATE TABLE `" . $data['db_prefix'], $sql);
						$sql = str_replace("INSERT INTO `if_", "INSERT INTO `" . $data['db_prefix'], $sql);
			...
			$db->query("INSERT INTO `" . $data['db_prefix'] . "setting` SET `group` = 'config', `key` = 'config_url', value = '" . $db->escape(HTTP_INVOICEFLASH) . "'");
