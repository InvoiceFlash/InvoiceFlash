...
class ModelUserUser extends Model {
	public function addUser($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "user` SET username = '" . $this->db->escape($data['username']) . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', cost_per_hour = '" . (float)(isset($data['cost_per_hour']) ? preg_replace('/[^0-9\.]/', '', $data['cost_per_hour']) : 0) . "', user_group_id = '" . (int)$data['user_group_id'] . "', status = '" . (int)$data['status'] . "', image = '" . $this->db->escape($data['image']) . "', language_id = " . (int)$data['language_id'] . ", signature = '" . $this->db->escape(isset($data['signature']) ? $data['signature'] : '') . "', date_added = NOW()");
	}

	public function editUser($user_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "user` SET username = '" . $this->db->escape($data['username']) . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', cost_per_hour = '" . (float)(isset($data['cost_per_hour']) ? preg_replace('/[^0-9\.]/', '', $data['cost_per_hour']) : 0) . "', user_group_id = '" . (int)$data['user_group_id'] . "', status = '" . (int)$data['status'] . "', image = '" . $this->db->escape($data['image']) . "', language_id = " . (int)$data['language_id'] . ", signature = '" . $this->db->escape(isset($data['signature']) ? $data['signature'] : '') . "' WHERE user_id = '" . (int)$user_id . "'");
