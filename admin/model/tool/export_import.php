...
class ModelToolExportImport extends Model {
	...
	protected function getManufacturers() {
		...
		$sql = "SELECT m2s.manufacturer_id, m2s.store_id, m.name AS name FROM " . DB_PREFIX . "manufacturer m";
	...
	protected function storeCustomerIntoDatabase(&$customer, &$available_customer_ids, &$available_address_ids) {
		...
		$sql = "INSERT INTO `" . DB_PREFIX . "customer` (`customer_id`,`store_id`,`firstname`,`lastname`,`email`,`telephone`,`fax`,`password`,`salt`,`cart`,`wishlist`,`newsletter`,`address_id`,`customer_group_id`,`ip`,`status`,`approved`,`token`,`date_added`) VALUES";
		$sql .= " ( '$customer_id', '$store_id', '$firstname', '$lastname', '$email', '$telephone', '$fax', '$password', '$salt', '$cart', '$wishlist', '$newsletter', '$address_id', '$customer_group_id', '$ip', '$status', '$approved', '$token', '$date_added');";
	...
	protected function validateCustomers($reader) {
		...
		$expected_heading = array(
			"customer_id", "store_id", "firstname", "lastname", "email", "telephone", "fax", "password", "salt", "cart", "wishlist", "newsletter", "address_id", "company", "company_id", "tax_id", "address_1", "address_2", "city", "postcode", "country_id", "zone_id", "customer_group_id", "ip", "status", "approved", "token", "date_added"
		);
	...
	protected function validateProducts($reader) {
		...
		$expected_heading = array_merge($expected_heading, array("location", "quantity", "model", "manufacturer_name", "image_name", "shipping", "price", "points", "date_added"));
		$expected_heading = array_merge($expected_heading, array("date_modified", "date_available", "weight", "weight_unit", "length", "width", "height", "length_unit", "status", "tax_class_id", "seo_keyword"));
	...
	protected function getManufacturerDescriptions($language_id, $language_code, $offset = null, $rows = null, $min_id = null, $max_id = null) {
		...
		$sql = "SELECT p.product_id, m.name AS manufacturer_name FROM " . DB_PREFIX . "manufacturer m";
		...
		$manufacturer_description = $manufacturer_query->rows;
	...
	public function getProducts($languages, $default_language_id, $product_fields, $offset = null, $rows = null, $min_id = null, $max_id = null) {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "manufacturer md ON (md.manufacturer_id = p.manufacturer_id)";
