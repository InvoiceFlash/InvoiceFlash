...
class ModelSaleCustomer extends Model {
	...
	public function addCustomer($data) {
		$sql = "INSERT INTO " . DB_PREFIX . "customer SET
			...
			customer_group_id = '" . (int)(isset($data['customer_group_id']) ? $data['customer_group_id'] : 0) . "',
		...
		$vat_regime = in_array(isset($data['vat_regime']) ? $data['vat_regime'] : '', array('general', 'comunitario', 'internacional')) ? $data['vat_regime'] : 'general';

		$this->db->query("UPDATE " . DB_PREFIX . "customer SET bank_cc = '" . $this->db->escape($bank_cc) . "', bic = '" . $this->db->escape($data['bic']) . "', efaccafi = '" . $this->db->escape($data['efaccafi']) . "', efaccafi_address = '" . $this->db->escape(isset($data['efaccafi_address']) ? $data['efaccafi_address'] : '') . "', efaccafi_city = '" . $this->db->escape(isset($data['efaccafi_city']) ? $data['efaccafi_city'] : '') . "', efaccafi_province = '" . $this->db->escape(isset($data['efaccafi_province']) ? $data['efaccafi_province'] : '') . "', efaccafi_postcode = '" . $this->db->escape(isset($data['efaccafi_postcode']) ? $data['efaccafi_postcode'] : '') . "', efaccare = '" . $this->db->escape($data['efaccare']) . "', efaccare_address = '" . $this->db->escape(isset($data['efaccare_address']) ? $data['efaccare_address'] : '') . "', efaccare_city = '" . $this->db->escape(isset($data['efaccare_city']) ? $data['efaccare_city'] : '') . "', efaccare_province = '" . $this->db->escape(isset($data['efaccare_province']) ? $data['efaccare_province'] : '') . "', efaccare_postcode = '" . $this->db->escape(isset($data['efaccare_postcode']) ? $data['efaccare_postcode'] : '') . "', efaccapa = '" . $this->db->escape($data['efaccapa']) . "', efaccapa_address = '" . $this->db->escape(isset($data['efaccapa_address']) ? $data['efaccapa_address'] : '') . "', efaccapa_city = '" . $this->db->escape(isset($data['efaccapa_city']) ? $data['efaccapa_city'] : '') . "', efaccapa_province = '" . $this->db->escape(isset($data['efaccapa_province']) ? $data['efaccapa_province'] : '') . "', efaccapa_postcode = '" . $this->db->escape(isset($data['efaccapa_postcode']) ? $data['efaccapa_postcode'] : '') . "', nif = '" . $this->db->escape($nif) . "', contable_account = '" . $this->db->escape(isset($data['contable_account']) ? $data['contable_account'] : '') . "', vat_regime = '" . $this->db->escape($vat_regime) . "', tariff_id = '" . (int)(isset($data['tariff_id']) ? $data['tariff_id'] : 0) . "', credit_limit = '" . (float)(isset($data['credit_limit']) ? preg_replace('/[^0-9\.]/', '', $data['credit_limit']) : 0) . "', cwww = '" . $this->db->escape($data['web']) . "', address = '" . $this->db->escape($data['address']) . "', city = '" . $this->db->escape($data['city']) . "', postcode = '" . $this->db->escape($data['postcode']) . "', country_id = '" . (int)$data['country_id'] . "', zone_id = '" . (int)$data['zone_id'] . "', customer_representative_id = " . (empty($data['customer_representative_id']) ? 'NULL' : (int)$data['customer_representative_id']) . ", nusuultmod = '" . (int)$this->user->getID() . "' WHERE customer_id = " . (int)$customer_id);

		if (isset($data['customer_address'])) {

			foreach ($data['customer_address'] as $address) {
	...
	public function editCustomer($customer_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET
			company = '" . $this->db->escape($data['company']) . "',
			notes = '', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', newsletter = '" . (int)$data['newsletter'] . "', customer_group_id = '" . (int)(isset($data['customer_group_id']) ? $data['customer_group_id'] : 0) . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE customer_id = '" . (int)$customer_id . "'");

		$bank_cc = str_replace(" ", "", $data['bank_cc']);
		$nif = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $data['nif']));
		$vat_regime = in_array(isset($data['vat_regime']) ? $data['vat_regime'] : '', array('general', 'comunitario', 'internacional')) ? $data['vat_regime'] : 'general';

		$this->db->query("UPDATE " . DB_PREFIX . "customer SET bank_cc = '" . $this->db->escape($bank_cc) . "', bic = '" . $this->db->escape($data['bic']) . "', efaccafi = '" . $this->db->escape($data['efaccafi']) . "', efaccafi_address = '" . $this->db->escape(isset($data['efaccafi_address']) ? $data['efaccafi_address'] : '') . "', efaccafi_city = '" . $this->db->escape(isset($data['efaccafi_city']) ? $data['efaccafi_city'] : '') . "', efaccafi_province = '" . $this->db->escape(isset($data['efaccafi_province']) ? $data['efaccafi_province'] : '') . "', efaccafi_postcode = '" . $this->db->escape(isset($data['efaccafi_postcode']) ? $data['efaccafi_postcode'] : '') . "', efaccare = '" . $this->db->escape($data['efaccare']) . "', efaccare_address = '" . $this->db->escape(isset($data['efaccare_address']) ? $data['efaccare_address'] : '') . "', efaccare_city = '" . $this->db->escape(isset($data['efaccare_city']) ? $data['efaccare_city'] : '') . "', efaccare_province = '" . $this->db->escape(isset($data['efaccare_province']) ? $data['efaccare_province'] : '') . "', efaccare_postcode = '" . $this->db->escape(isset($data['efaccare_postcode']) ? $data['efaccare_postcode'] : '') . "', efaccapa = '" . $this->db->escape($data['efaccapa']) . "', efaccapa_address = '" . $this->db->escape(isset($data['efaccapa_address']) ? $data['efaccapa_address'] : '') . "', efaccapa_city = '" . $this->db->escape(isset($data['efaccapa_city']) ? $data['efaccapa_city'] : '') . "', efaccapa_province = '" . $this->db->escape(isset($data['efaccapa_province']) ? $data['efaccapa_province'] : '') . "', efaccapa_postcode = '" . $this->db->escape(isset($data['efaccapa_postcode']) ? $data['efaccapa_postcode'] : '') . "', nif = '" . $this->db->escape($nif) . "', contable_account = '" . $this->db->escape(isset($data['contable_account']) ? $data['contable_account'] : '') . "', digital_invoice = '" . (int)$data['digital_invoice'] . "', vat_regime = '" . $this->db->escape($vat_regime) . "', tariff_id = '" . (int)(isset($data['tariff_id']) ? $data['tariff_id'] : 0) . "', credit_limit = '" . (float)(isset($data['credit_limit']) ? preg_replace('/[^0-9\.]/', '', $data['credit_limit']) : 0) . "', cwww = '" . $this->db->escape($data['web']) . "', address = '" . $this->db->escape($data['address']) . "', city = '" . $this->db->escape($data['city']) . "', postcode = '" . $this->db->escape($data['postcode']) . "', country_id = '" . (int)$data['country_id'] . "', zone_id = '" . (int)$data['zone_id'] . "', customer_representative_id = " . (empty($data['customer_representative_id']) ? 'NULL' : (int)$data['customer_representative_id']) . ", nusuultmod = '" . (int)$this->user->getID() . "' WHERE customer_id = " . (int)$customer_id);
	...
	public function getCustomer($customer_id) {
		...
		$query = $this->db->query("SELECT c.company, c.email, c.nif, c.telephone, c.cwww, c.newsletter, c.customer_group_id, c.status, c.bank_cc, c.bic, c.efaccafi, c.efaccafi_address, c.efaccafi_city, c.efaccafi_province, c.efaccafi_postcode, c.efaccapa, c.efaccapa_address, c.efaccapa_city, c.efaccapa_province, c.efaccapa_postcode, c.efaccare, c.efaccare_address, c.efaccare_city, c.efaccare_province, c.efaccare_postcode, c.contable_account, c.digital_invoice, c.vat_regime, c.credit_limit, c.address, c.city, c.postcode, c.country_id, c.zone_id, c.customer_representative_id, c.date_added, c.date_modified, c.notes, c.address_id, c.store_id, u.username AS last_modified_by FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "user u ON u.user_id = c.nusuultmod WHERE c.customer_id = '" . (int)$customer_id . "'");

		return $query->row;

	}

	// Deuda pendiente de cobro del cliente: suma de los recibos (if_receipt) aun
	// no cobrados de sus facturas de venta — usado por el aviso de limite de
	// credito en sale/order (ver ControllerSaleOrder::checkCreditLimit()).
	public function getCustomerOutstandingBalance($customer_id) {
		$query = $this->db->query("SELECT SUM(r.amount) AS total FROM " . DB_PREFIX . "receipt r LEFT JOIN " . DB_PREFIX . "invoice i ON r.invoice_id = i.invoice_id WHERE i.customer_id = '" . (int)$customer_id . "' AND r.paid = 0");

		return $query->num_rows ? (float)$query->row['total'] : 0;
	}
