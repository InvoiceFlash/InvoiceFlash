...
class ModelSaleCustomer extends Model {
	...
	public function editCustomer($customer_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET 
			firstname = '" . $this->db->escape($data['firstname']) . "',notes = '', 
			lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', newsletter = '" . (int)$data['newsletter'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', status = '" . (int)$data['status'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function getEmailsByCustomerId($customer_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "fl_mails` WHERE type = 'R' AND bleido != 2 AND customer_id=" . (int)$customer_id . " ORDER BY date_added DESC");
	...
	public function getCustomerContracts ($customer_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "fl_contracts WHERE ncliente = " . (int)$customer_id);

		return $query->rows;
	}

	public function getCustomerContract($contract_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "fl_contracts WHERE nid = " . (int)$contract_id);
	...
	public function editCustomerContract($data, $contract_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "fl_contracts SET 
			...
			caplultmod = 'web' WHERE nid = " . (int)$contract_id);
	}

	public function deleteCustomerContract($contract_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "fl_contracts WHERE nid = " . (int)$contract_id);
	}
 	...
 	function getCustomerNote($note_id) {
 		$sql = "SELECT ch.comment, ch.date_added, ch.user_id, u.Login FROM customer_history ch LEFT JOIN gpu_user u ON ch.user_id = u.UserId WHERE ch.customer_history_id = " . (int)$note_id;
