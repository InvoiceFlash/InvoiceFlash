...
class ModelSaleOrder extends Model {
	...
	public function createDelivery($order_id) {
		$quote_info = $this->getOrder($order_id);

		if ($quote_info && !$quote_info['invoice_no']) {

			$quote_products = $this->getOrderProducts($order_id);

			$order_products = array();		
		
			foreach ($quote_products as $quote_product) {
				
				$quote_option = $this->getOrderOptions($order_id, $quote_product['order_product_id']);
												
				$order_products[] = array(
					'delivery_product_id' => $quote_product['order_product_id'],
			...
			$data = array(
				...
				'delivery_status_id' => $quote_info['order_status_id'],
