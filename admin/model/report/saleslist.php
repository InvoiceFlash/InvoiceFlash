...
class ModelReportSaleslist extends Model {
	...
	public function getDeliverys($data = array()) { 
		...
		$sql = "select o.delivery_id,o.email,o.telephone,CONCAT(o.shipping_address_1, ' ', o.shipping_address_2) AS address,CONCAT(o.firstname, ' ', o.lastname) AS customer,o.shipping_zone AS state,o.shipping_postcode AS postcode,(SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.delivery_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS  status,o.shipping_city AS city,GROUP_CONCAT(pd.name) AS pdtname,sum(op.quantity) AS quantity,GROUP_CONCAT(opt.value ) AS options, GROUP_CONCAT(opt.delivery_product_id )  AS ordprdid,GROUP_CONCAT(op.delivery_product_id )  AS optprdid, GROUP_CONCAT(op.quantity)   AS opquantity from `" . DB_PREFIX . "delivery` o LEFT JOIN " . DB_PREFIX . "delivery_product op ON (op.delivery_id = o.delivery_id)  LEFT JOIN " . DB_PREFIX . "product_description pd ON (pd.product_id = op.product_id and pd.language_id = '" . (int)$this->config->get('config_language_id') . "') LEFT JOIN " . DB_PREFIX . "delivery_option opt ON (opt.delivery_product_id = op.delivery_product_id) ";
	...
	public function getInvoices($data = array()) { 
		...
		$sql = "select o.invoice_id,o.email,o.telephone, o.payment_city AS city,o.shipping_company AS customer,o.date_added AS date_added,o.shipping_postcode AS postcode, (SELECT os.name FROM " . DB_PREFIX . "invoice_status os WHERE os.invoice_status_id = o.invoice_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS  status from `" . DB_PREFIX . "invoice` o ";
