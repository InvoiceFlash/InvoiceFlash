...
class ControllerToolPendingInvoices extends Controller {
	...
	public function incorporate() {
		...
		if (!is_file(DIR_APPLICATION . 'model/purchase/supplier.php') || !is_file(DIR_APPLICATION . 'model/purchase/invoice.php')) {
			$this->session->data['error'] = $this->language->get('error_permission');
			$this->redirect($this->url->link('tool/pending_invoices', 'token=' . $this->session->data['token'], 'SSL'));
			return;
		}
		...
		// Contabilización automática (si el módulo Contabilidad está instalado
		// y "Activar contabilidad" está en Sí - postPurchaseInvoice() ya hace
		// esa comprobación internamente) - mismo gancho que ya tiene
		// ControllerPurchaseInvoice::insert()/update() al guardar a mano.
		if (is_file(DIR_APPLICATION . 'model/accounting/auto_entry.php')) {
