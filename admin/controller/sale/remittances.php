...
class ControllerSaleRemittances extends Controller {
	private $error = array();
  	...
  	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_invoice->editOrder($this->request->get['order_id'], $this->request->post);
  	...
  	public function delete() {
    	...
    	if (isset($this->request->post['selected']) && ($this->validateDelete())) {
			foreach ($this->request->post['selected'] as $order_id) {
				$this->model_sale_remittances->deleteOrder($order_id);
			}
  	...
  	private function getList() {
		...
		$this->data['invoice'] = $this->url->link('sale/remittances/invoice', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['insert'] = $this->url->link('sale/remittances/insert', 'token=' . $this->session->data['token'], 'SSL');
		...
		$order_total = $this->model_sale_remittances->getTotalOrders($data);

		$results = $this->model_sale_remittances->getOrders($data);
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('sale/remittances/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['remittance_id'] . $url, 'SSL')
			);
			
			// if (strtotime($result['date_added']) > strtotime('-' . (int)$this->config->get('config_order_edit') . ' day')) {
				// $action[] = array(
					// 'text' => $this->language->get('text_edit'),
					// 'href' => $this->url->link('sale/remittances/update', 'token=' . $this->session->data['token'] . '&order_id=' . $result['remittance_id'] . $url, 'SSL')
				// );
			// }
			
			$this->data['orders'][] = array(
				'order_id'      => $result['remittance_id'],
  	...
  	public function getForm() {
		...
		if (!isset($this->request->get['order_id'])) {
			...
			$this->data['action'] = $this->url->link('sale/invoice/update', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . $url, 'SSL');
		}
		...
		$this->data['order_statuses'] = $this->model_localisation_invoice_status->getOrderStatuses();	
		...
		if (isset($this->request->post['order_voucher'])) {
			...
			$this->data['order_vouchers'] = $this->model_sale_invoice->getOrderVouchers($this->request->get['order_id']);			
		} else {
  	...
  	private function validateGenerate() {
	...
	public function info() {
		...
		$lineas = $this->model_sale_remittances->getRemittances($order_id);
		...
		foreach ($lineas as $line) {
			...
			$this->data['receipts'][] = array(
					...
					'vat_customer' => $line['customer_vat'],
		...
		$this->data['text_email'] = $this->language->get('text_email');
			...
			$this->data['breadcrumbs'][] = array(
				'text'      => $this->language->get('heading_title_lines'),
				'href'      => $this->url->link('sale/remittances', 'token=' . $this->session->data['token'] . $url, 'SSL'),				
				'separator' => ' :: '
			);
			...
			$remittances_total = $this->model_sale_remittances->getRemittancesTotal();
			
			$pagination = new Pagination();
			$pagination->total = $remittances_total;
			$pagination->page = $page;
			$pagination->limit = $this->config->get('config_admin_limit');
			$pagination->text = $this->language->get('text_pagination');
