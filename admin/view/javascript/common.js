...
$(document).ready(function() {
	...
	$('input[name="author_name"]').typeahead({
		source:function(q,process){
			return $.getJSON('index.php?route=simple_blog/author/autocomplete&token='+token+'&filter_name='+encodeURIComponent(q),function(json){
				var data=[];
				$.each(json,function(i,item){
					mapped[item.name]=item.simple_blog_author_id;
					data.push(item.name);
				});
				process(data);
			});
		...
		updater:function(item){
			$('input[name="simple_blog_author_id"]').val(mapped[item]);
			return item;
		}
	});
