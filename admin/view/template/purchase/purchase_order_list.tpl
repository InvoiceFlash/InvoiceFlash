			...
			<table class="table table-bordered table-striped table-hover">
				<thead>
					<tr>
						...
						<th class="text-right"><a href="<?php echo $sort_purchase_order_id; ?>"><?php echo $column_purchase_order_id; echo ($sort == 'po.purchase_order_id') ? '<i class="caret caret-' . strtolower($order) . '"></i>' : ''; ?></a></th>
						...
						<th class="text-right hidden-xs"><a href="<?php echo $sort_total; ?>"><?php echo $column_total; echo ($sort == 'po.total') ? '<i class="caret caret-' . strtolower($order) . '"></i>' : ''; ?></a></th>
						<th class="hidden-xs"><a href="<?php echo $sort_date_added; ?>"><?php echo $column_date_added; echo ($sort == 'po.date_added') ? '<i class="caret caret-' . strtolower($order) . '"></i>' : ''; ?></a></th>
						<th class="hidden-xs hidden-sm"><a href="<?php echo $sort_date_modified; ?>"><?php echo $column_date_modified; echo ($sort == 'po.date_modified') ? '<i class="caret caret-' . strtolower($order) . '"></i>' : ''; ?></a></th>
						<th class="text-right"><span class="hidden-xs"><?php echo $column_action; ?></span></th>
					</tr>
				</thead>
				<tbody data-link="row" class="rowlink">
					<tr id="filter" class="info">
						<td class="text-center"><a class="btn btn-default btn-block" href="index.php?route=purchase/purchase_order&token=<?php echo $token; ?>" rel="tooltip" title="Reset"><i class="fa fa-power-off fa-fw"></i></a></td>
						<td class="text-right"><input type="text" name="filter_purchase_order_id" value="<?php echo $filter_purchase_order_id; ?>" class="form-control text-right"></td>
						<td><input type="text" name="filter_supplier" value="<?php echo $filter_supplier; ?>" class="form-control"></td>
						<td class="hidden-xs"><select name="filter_purchase_order_status_id" class="form-control">
							<option value="*">&ndash;</option>
							<?php if ($filter_purchase_order_status_id == '0') { ?>
							<option value="0" selected=""><?php echo $text_missing; ?></option>
							<?php } else { ?>
							<option value="0"><?php echo $text_missing; ?></option>
							<?php } ?>
							<?php foreach ($purchase_order_statuses as $purchase_order_status) { ?>
							<?php if ($purchase_order_status['purchase_order_status_id'] == $filter_purchase_order_status_id) { ?>
					...
					<tr>
						<td class="rowlink-skip text-center"><?php if ($purchase_order['selected']) { ?>
							<input type="checkbox" name="selected[]" value="<?php echo $purchase_order['purchase_order_id']; ?>" checked="">
							<?php } else { ?>
						...
						<td><?php echo $purchase_order['supplier']; ?></td>
						<td class="hidden-xs"><?php echo $purchase_order['status']; ?></td>
						<td class="text-right hidden-xs"><?php echo $purchase_order['total']; ?></td>
						<td class="hidden-xs"><?php echo $purchase_order['date_added']; ?></td>
						<td class="hidden-xs hidden-sm"><?php echo $purchase_order['date_modified']; ?></td>
						<td class="text-right"><?php foreach ($purchase_order['action'] as $action) { ?>
