...
<div class="panel panel-default">
	<?php $fa = 'shipping-fast'; include(DIR_TEMPLATE . 'common/template-title-list.tpl'); ?>
		...
		<form class="form-inline" action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form">
			<table class="table table-bordered table-striped table-hover">
				...
				<tbody data-link="row" class="rowlink">
					...
					<tr>
						<td class="rowlink-skip text-center"><?php if ($shipping['selected']) { ?>
							<input type="checkbox" name="selected[]" value="<?php echo $shipping['shipping_id']; ?>" checked="">
							<?php } else { ?>
							<input type="checkbox" name="selected[]" value="<?php echo $shipping['shipping_id']; ?>">
							<?php } ?></td>
						<td><?php echo $shipping['name']; ?></td>
						<td class="text-right"><?php foreach ($shipping['action'] as $action) { ?>
